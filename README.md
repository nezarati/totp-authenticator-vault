...
👉 **[https://nezarati.github.io/totp-authenticator-vault/](https://nezarati.github.io/totp-authenticator-vault/)** 👈
...
- **Notes field** with Markdown support: bold, italic, inline code (click to copy), and `#tags` (click to filter).

### Import & Export (Encrypted JSON)

- Export your **entire vault** (TOTP + passwords) to a **password‑protected JSON file**.
- Encryption: **AES‑256‑GCM** with **PBKDF2** (150k iterations, SHA‑256).
...
- Each login becomes a credential entry in the password manager.
...
- **Dark / Light theme** – follows system preference by default, with a manual toggle (moon/sun icon).
